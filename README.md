...
- National map of ~400 active fires: pins, perimeters (pre-rendered snapshot image), satellite hotspot detections with age-based styling
- Per-fire view: versioned perimeter history, stats, AI incident summary, structure exposure
- Spread forecasts (pyrecast ELMFIRE): crown fire, flame length, spread rate, hours-since-burned, time of arrival — percentile selector (50/90 pre-rendered), hourly playback out to +7 days
...
Browser (Vite+React+TS+MapLibre SPA — GitHub Pages)
 ...
 └─→ https://fNNN.backblazeb2.com/file/responder-debrief-data/…   catalogs, frames, tiles, PDFs (B2, CORS *)
Cron worker (Python 3.12 + gdal-bin CLI, GitHub Actions, public repo)
 ├─ catalogs job (hourly):   fire API + pyrecast caps → pre-rendered WMS frames + catalog.json / pyrecast_runs.json / weather_runs.json
 └─ mirror job (4×/day):     FTP crawl → match to active fires → mirror PDFs → GeoPDF→XYZ tiles → B2
```

**Static frames**: the browser never talks to the pyrecast geoservers. The hourly catalogs job pre-renders every WMS image the frontend needs — spread-forecast frames per new run (percentiles **50/90** only, thinned time instants, run-bbox PNGs), CONUS weather frames per HRRR forecast hour, a national perimeters snapshot, and legends — into B2 as static, mostly immutable PNGs that the manifests reference by path template. Each frame is fetched from pyrecast exactly once ever (runs are immutable and run-stamped), with ≤2 concurrent requests per geoserver and a per-sync image budget (`FRAME_BUDGET`, default 3000; budget-limited runs are marked `"complete": false` and finish on later ticks).
...
**Python worker** (dry-run writes catalogs and frames to `./out/` instead of B2 — no credentials needed):

```sh
cd worker
uv sync
uv run python -m responder_worker.cli sync-catalogs --dry-run \
  --frames-fires <slug> --frames-hours 8 --frames-products smoke,ws,rh   # polite frame limits
uv run python -m responder_worker.cli sync-incidents --dry-run --fire elk
uv run pytest        # unit tests against tests/fixtures/
```

Dry-run frames land under `worker/out/frames/…`; copy `worker/out/` into `frontend/public/data` for local dev data.
