...
- **Incident maps tab**: grouped by product kind (ops/iap/brief/pio/evac/airops/mobile/other), newest badged "Latest", thumbnail previews; georeferenced → exclusive "Show on map" toggle + opacity slider (default 0.75) + zoom-to-bounds; non-geo → "opens as PDF" pill with size warning (26 MB Avenza). One map overlay at a time: a sheet (or series) and an IR flight replace each other; each is dismissed from its own row. IR flights file under their day and render as a GeoJSON heat toggle (`irHeatLayer`) in the IR PDF's symbology.
...
4. **IR vectors** (`ir_vectors.py`): Shapefiles.zip (or the KMZ when a team publishes only that) → `ogr2ogr -t_srs EPSG:4326` per heat class → merged FeatureCollection tagged `heat_type` (Perimeter/Intense/Scattered/Isolated, plus Obscured and Possible), + `Estimated Acreage` from Read_Me, + the flight time the KMZ states → `vectors/ir/{fire_slug}/{flight}.geojson`.
